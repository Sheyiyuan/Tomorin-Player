...
| Windows | Cross-compile and NSIS generation in CI | Windows runner performs silent install, version check, and uninstall |
| macOS | Universal app, required DMG generation, and `Info.plist` checks in CI | macOS runner launches the `.app` and verifies the application remains running |
